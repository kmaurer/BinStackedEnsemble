...
make_bin_fitted_weights <- function(bin_fitted_model_list, bin_train,train_data, bin_test,test_data){
  ...
  for(b in 1:B){
      ...
      preds[in_bin_b, ] <- make_preds(data=bin_train_b,model_storage_list = bin_fitted_model_list[[b]],true_classes=true_classes)
  }
  preds <- sapply(paste0("preds",1:M), function(x) factor(preds[,x], labels=true_classes))
...
make_bin_fitted_model_metric_array <- function(combination_rule, bin_fitted_model_list, test_data, bin_test, true_classes){
  ...
  if(combination_rule == "majority vote"){
    ...
    for(j in 1:ncol(test_preds)){
      test_preds[,j] <- factor(test_preds[,j], labels=true_classes)
    }
...
predict_bin_fitted_ensemble <- function(model_types, bin_features, bin_type, nbins, train_data, test_data, comb_rule, weightType){
  ...
  modelWeights <- make_bin_fitted_weights(bin_fitted_model_list, bin_train,train_data, bin_test,test_data)
...
iq_bin_fitted_testing <- function(train_data, test_data, model_types, bin_features_list, nbins_list){
...
(nbins_list <- make_nbins_list_pairs(2:3))
(bin_features_list <- make_bin_feature_list_pairs(c("Aa","Ao","Iy"), ordered=FALSE))
